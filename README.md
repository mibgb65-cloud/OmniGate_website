...
项目的核心目标是把“账号管理”、“任务编排”、“浏览器自动化执行”和“实时日志回显”串成一条完整链路。当前仓库中，Google 与 ChatGPT 模块已经具备可落地的任务执行链路，GitHub 模块当前以账号池管理能力为主。
...
- ChatGPT 自动注册任务投递、轮询、账号入池与 2FA 自动绑定
- CloudMail 配置中心与验证码邮件拉取链路
- Google / GitHub / ChatGPT 账号导出与 2FA 工具页批量解析
...
- `omnigate-chatgpt`: ChatGPT 账号池、自动注册任务、2FA 与任务状态联动
- `omnigate-github`: GitHub 账号池与详情管理

其中 Google 与 ChatGPT 模块都已经包含“前端触发 -> 后端投递 -> Worker 执行 -> 数据回写”的完整参考链路。
...
如果要使用 ChatGPT 自动注册，需要先登录管理后台并完成 CloudMail 配置：

- 页面入口：`/settings/cloudmail`
- 必填项：
  - `cloudmail.account_email`
  - `cloudmail.password`
  - `cloudmail.auth_url`
  - `chatgpt.registration_email_suffix`

如果这些配置缺失或格式不正确，后端会在投递自动注册任务前直接拦截并返回错误，不会把失败任务投递到 Worker。
...
```bash
cd omnigate_worker
python tests/manual/manual_test_batch_register_chatgpt_accounts.py --signup-count 1
```

默认会显示浏览器窗口，并在流程结束后保持打开，便于人工检查页面状态。
