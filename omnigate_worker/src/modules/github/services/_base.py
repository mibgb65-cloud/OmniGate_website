...
from src.modules.github.models import GithubAccountCredential
...
class GithubServiceBase:
    """GitHub 账号服务的公共依赖与辅助能力。"""
    ...
    def __init__(
        ...
        *,
        ...
        if account_repository is None:
            ...
            account_repository = GithubAccountRepository(db_pool)
        if persistence is None:
            if db_pool is None:
                raise ValueError("db_pool or persistence is required")
            persistence = GithubAccountPersistence(db_pool)
    ...
    async def _load_credential(self, account_id: int) -> GithubAccountCredential:
        ...
        if not email:
            raise ValueError(f"GitHub 账号邮箱为空: account_id={account_id}")
        if not password:
            raise ValueError(f"GitHub 账号密码为空: account_id={account_id}")
