...
DEFAULT_TARGET_EMAIL = "colcloughbruno600@gmail.com"


class ManualTestInviteGoogleFamilyMemberByAccountId:
    """Manual test: login by account id, then invite target email."""
        ...
        try:
            result = await service.execute(
                InviteGoogleFamilyMemberByAccountIdParams(
                    ...
                    invited_account_email=self.target_email,
                ),
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Manual test: invite family member by account id")
    parser.add_argument(
        "--account-id",
        type=int,
    ...
    parser.add_argument(
        ...
        help=f"target email to invite (default: {DEFAULT_TARGET_EMAIL})",
    )
    parser.add_argument(
        "--keep-page-open",
        action="store_true",
...
async def _main_async() -> None:
    ...
    runner = ManualTestInviteGoogleFamilyMemberByAccountId(
